...
def wastewater_model(
    N0, N, n_SI, SI, n_v, v, M, viral, viral_obs, EpidemicStart, nsd, sampleday
):
    ...
    for i in range(N0, N):
        convolution = jnp.sum(
            prediction[i - j - 1] * SI[j - 1] for j in range(1, min(n_SI, i - 1) + 1)
        )
    ...
    for i in range(1, N):
        E_viral = E_viral.at[i].set(
            jnp.sum(
                prediction[i - j - 1] * v[j - 1] for j in range(1, min(n_v, i - 1) + 1)
            )
        )
    ...
    with numpyro.plate("obs", nsd):
        numpyro.sample(
            "obs",
            dist.Normal(jnp.log(E_viral[sampleday]), nu[sampleday]),
            obs=viral[sampleday, : viral_obs[sampleday]],
        )

    # Update Rt for t > 1
    for t in range(1, N):
        Rt = Rt.at[t].set(
            numpyro.sample(f"Rt_{t}", dist.LogNormal(jnp.log(Rt[t - 1]), sigma))
        )
